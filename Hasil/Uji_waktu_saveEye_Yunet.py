...
classifier = tflite.Interpreter(model_path="Class_model_CNN3_15.tflite")
classifier.allocate_tensors()
input_classifier = classifier.get_input_details()
output_classifier = classifier.get_output_details()

#Model prediksi kantuk
drowsy_pred = tflite.Interpreter(model_path = "Pred_model_CNN2_15.tflite")
...
out_vid = cv2.VideoWriter('./DATA_Vid/tes24Juni.mp4', fourcc, 18, (320,240))
graph_vid = cv2.VideoWriter('./DATA_Vid/tes24Juni_grafik.mp4', fourcc, 18, (580,380))
...
while True:
    ...
    if results is not None:
        ...
        if idxS==1:
            if (time.time() - t_baru) >= 15:    #Apabila proses telah berjalan selama 1 menit
        ...
        #Proses data pada menit kedua dan seterusnya
            ...
            if (time.time() - t_baru) >= 15: #apabila proses telah berjalan selama 1 menit
                ...
                perclos_time += (time.time()-t9)

                print('preproc time {}'.format(preproc_time/n_ear_mata))
                print('dacedet time {}'.format(facedet_time/n_ear_mata))
                print('convert time {}'.format(conver_time/n_ear_mata))
                print('landmarkpred time {}'.format(landmark_time/n_ear_mata))
                print('cal ear time {}'.format(calear_time/n_ear_mata))
                print('blink_class time {}'.format(blink_class_time/n_ear_mata))
                print('showvid time {}'.format(show_vid_time/n_ear_mata))
                print('perclos time {}'.format(perclos_time/idxS))
                print('drowsy class time {}'.format(class_drow_time/idxS))
                print('pred time {}'.format(pred_drow_time/idxS))
...
#    graph_vid.write(imgPlotEye)
    ...
    if key == ord("q" or "Q"): #Tombol untuk menghentikan proses
        df.to_csv('./DATA_file/tes24jun' + '.csv', sep=';', encoding='utf-8', decimal=',',
        index=False, mode='a')
        break
